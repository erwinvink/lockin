...
final class TrainingEngineTests: XCTestCase {
    ...
    func testGeneratesSessionsOnSelectedTrainingDays() throws {
        let calendar = Calendar.current
        let weekStart = try XCTUnwrap(calendar.date(from: DateComponents(year: 2026, month: 6, day: 1)))

        let plan = TrainingEngine().generateWeek(
            start: weekStart,
            weekIndex: 1,
            baseline: Baseline(pullUps: 5, pushUps: 20, plankSeconds: 60),
            preferences: TrainingPreferences(
        ...
        let offsets = plan.sessions.map {
            calendar.dateComponents([.day], from: calendar.startOfDay(for: weekStart), to: calendar.startOfDay(for: $0.date)).day
        }

        XCTAssertEqual(offsets, [0, 2, 5])
    }
    ...
    func testMissedSessionResetsConsistencyStreakImmediately() {
        let rank = RankState(consistencyScore: 40, streak: 6)
        let outcome = TrainingEngine().score(
            log: SessionLogInput(completed: false, pullUps: 0, pushUps: 0, plankSeconds: 0, rpe: 1, painLevel: 0, fatigueLevel: 1),
            plannedSession: nil
        )
