...
7. When `raceGoal.raceDate` falls on one of this week's offsets, make that day's session the race effort: title it with the race name, kind `long`, the race distance and elevation, and a goal-effort target. Keep every other run that week a short shakeout or recovery run.
8. Give every session a `target`: `pace` in seconds per km with `low` as the faster bound, or `hr` in bpm, so each run can be pushed to Garmin as a structured workout. Write `zone` values in the form "Zone 2" (capital Z, space, digit) so app display stays consistent.
9. Write `summary`, `purpose`, and `notes` in the calm coach voice.
10. Return JSON only, matching `running-week.schema.json`.
