...
test("garminSnapshot combines status, wellness, and activities", async (t) => {
  ...
  assert.deepEqual(
    ...
    [
      "http://127.0.0.1:8788/activities?days=90",
      "http://127.0.0.1:8788/status",
...
test("garminSnapshot clamps sinceDays per endpoint", async (t) => {
  ...
  await garminSnapshot(90, fetchImpl, { activityDays: 120 });
  ...
  assert.ok(urls.includes("http://127.0.0.1:8788/activities?days=90"));
  assert.ok(urls.includes("http://127.0.0.1:8788/wellness?days=1"));
  assert.ok(urls.includes("http://127.0.0.1:8788/activities?days=1"));
  // Activities default to a 90-day lookback regardless of the wellness window:
  ...
  assert.equal(urls.filter((u) => u.includes("/activities?days=90")).length, 2);
});
