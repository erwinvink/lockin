...
Check `GET https://lockin.elevenfactor.com/health` to confirm the proxy is running and has an API key. The app's Coach settings surface stores the selected model ID and can refresh available text model IDs through `GET https://lockin.elevenfactor.com/models`.
